...
class DefuntsController extends AppController {
    ...
    $personne = $this->Defunt->find('first',array(
        ...
        $this->Session->setFlash(__('Vous n\'êtes pas le propriétaire de cette personne'),"default", array('class' => 'alert-box warning round'));
        return $this->redirect(array('controller' => 'pages', 'action' => 'display','home'));
            
        }
    if (!$personne) {
        throw new NotFoundException(__('Erreur personne'));
    }
    ...
    public function delete($id = null,$idd = null) {
        ...
        if($this->Auth->password($this->request->data['Defunt']['password']) == $user['User']['password']){
            ...
            if ($this->Defunt->delete($id)) {
                ...
                $this->Session->setFlash(__('Le profil a bien été supprimé ainsi que ses contenus'),"default", array('class' => 'alert-box success radius'));
...
public function addall() {
        ...
        return $this->redirect(array('controller'=>'users','action'=>'account',$this->Auth->user('id')));
        }else{
                    $this->Session->setFlash(__('Vérifiez votre saisie'),"default", array('class' => 'alert-box warning round'));
    ...
    public function getallbyfamily($id = null){
        ...
        $this->loadModel('DefuntUsers');
        $defus = $this->DefuntUsers->find('all',array(
            'conditions'=>array('DefuntUsers.user_id'=>$this->Auth->user('id'))
            ));
        $personnes = array();
        foreach ($defus as $key => $value) {
            $def = $this->Defunt->find('first',array(
                'conditions'=>array('Defunt.id'=>$value['DefuntUsers']['defunt_id'])));
           if($def['Defunt']['family_id'] == $id){
            array_push($personnes, $def);
           }
        }
