...
class ClientPagesController extends AppController {
    ...
    public function liste($id = null){
        ...
        if (!$pages) {
            throw new NotFoundException(__('Erreur page'));
        }
    ...
    public function photo($id = null) {
        ...
        if(empty($medias)){
            $this->Session->setFlash(__('Page sans contenu ou inexistante'),"default", array('class' => 'alert-box warning round'));
            return $this->redirect(array('controller'=>'pages','action'=>'display','home'));
        }
        if(!empty($medias)){
            foreach ($medias as $key => $value) {
                if(empty($value['Media']['album_id'])) {
                    $mediasaf[$key]=$value;
                }
            }
            $alltags = array();
        ...
        if(!empty($this->request->data)){
            if($this->request->data['ClientPage']['password'] == $clientpage['ClientPage']['password']){
                ...
                $this->Session->setFlash(__('Ce n\'est pas le bon mot de passe'),"default", array('class' => 'alert-box warning rounds'));
            }
        }

        $this->set(compact('medias','mediasaf','clientpage','password','cats','personne','alltags','thumb'));  
    }
        ...
        function compare($a, $b)
{
                    ...
                    if ($this->request->data['ClientPage']['backgroundimg']['size'] <= 4097152){
                        ...
                        if ($ExtensionPresumee == 'jpg' || $ExtensionPresumee == 'jpeg'){
                            ...
                            if($ImageNews['mime'] == $ListeExtension[$ExtensionPresumee]  || $ImageNews['mime'] == $ListeExtensionIE[$ExtensionPresumee]){
                                ...
                                $this->Session->setFlash(__('Modifications sauvegardées'),"default", array('class' => 'alert-box success radius'));
        ...
        if(!empty($this->request->data)){
            $this->request->data['ClientPage']['id'] = $id;
                if($this->ClientPage->validates()){
                    ...
                    $this->Session->setFlash(__('Impossible à mettre à jour'),"default", array('class' => 'alert-box warning round'));
                    }
                }else{
                    $this->Session->setFlash(__('Vérifiez votre saisie'),"default", array('class' => 'alert-box warning round'));
                }
    ...
    public function viewcat($id = null,$idd) {
        ...
        if(empty($medias)){
            $this->Session->setFlash(__('Pas de contenus dans cette catégorie'),"default", array('class' => 'alert-box warning round'));
