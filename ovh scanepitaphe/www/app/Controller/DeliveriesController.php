    ...
    public function admin_add(){
		...
		if ($this->request->is('post')) {
			...
			if ($this->Delivery->save($this->request->data)) {
				$this->Session->setFlash(__('Votre mode de livraison a bien été ajouté'),"default", array('class' => 'alert-box success radius'));
	            return $this->redirect(array('controller'=>'deliveries', 'action' => 'liste'));
			}
			$this->Session->setFlash(__('Impossible d\'ajouter le mode de livraison'),"default", array('class' => 'alert-box warning round'));                
		}
    }
...
public function admin_edit ($id = null) {
		...
		if ($this->request->is(array('post','put'))) {
			...
			if ($this->Delivery->save($this->request->data)) {
				...
				$this->Session->setFlash(__('Mode de livraison mis à jour'),"default", array('class' => 'alert-box success radius'));
	    		$this->redirect(array('controller' => 'deliveries', 'action' => 'liste','admin'=>true));
	    	}else{
				$this->Session->setFlash(__('Impossible de mettre à jour le mode de livraison'),"default", array('class' => 'alert-box warning round'));			
			}
		}
	...
	public function admin_delete ($id = null) {
		...
		if ($this->Delivery->delete($id)) {
			$this->Session->setFlash(__('Mode de livraison supprimé'),"default", array('class' => 'alert-box success radius'));
	    	$this->redirect(array('controller' => 'deliveries', 'action' => 'liste','admin'=>true));
		}else{
				$this->Session->setFlash(__('Impossible de supprimer le mode de livraison'),"default", array('class' => 'alert-box warning round'));			
				$this->redirect(array('controller' => 'deliveries', 'action' => 'liste','admin'=>true));
			}
	}
