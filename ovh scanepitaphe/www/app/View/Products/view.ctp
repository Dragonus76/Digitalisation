		...
		<?php if($product['Product']['nupload'] == 1):?>
			<?php
		        ...
		        if(!empty($array)){echo $this->Form->input('defunt_id', array(
		        	...
		        	echo '<p>'.__('Vous devez ajouter une personne pour commencer.').'</p>';
		        }

				echo $this->Form->input('content', array('label' => __('2 - Ajouter un texte à imprimer sur la plaque'),'rows' => '3'));
				echo $this->Form->input('picture_file',array('label' => __('3 - Choisissez votre photo personnelle (mode portrait)'), 'type' => 'file')); 
				echo $this->Form->input('prodname',array('type'=>'hidden','value'=>$product['Product']['name']));
				echo $this->Form->submit(__('Personnaliser l\'apparence'),array('class'=>'buttonvioletokfull postfix')); 
            echo $this->Form->end();
            ?>
			
		<?php else: ?>
		<br/>
			<?php echo $this->Html->link(__('Ajouter au panier'), array('controller'=>'checkouts', 'action' => 'add_to_cart',  $product['Product']['id']),array('class' => 'buttonvioletokfull')); ?>
		<?php endif ?>
      </div>
    </div>
</div>
