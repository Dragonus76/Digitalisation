...
<div class="row">
	<div class="small-12 medium-12 large-12 columns" >
		...
		<?php
		if(isset($thumb)){
			echo '<h2>'; echo __('Les albums');echo '</h2>';
			foreach ($thumb as $key => $value) {
	
				echo '<div class="small-12 medium-3 large-3 columns left albumcov center">';
				echo $this->Html->image('medias/defunts/defunt_'.$value['Media']['defunt_id'].DS.$value['Media']['nomfichier'],array(
					'url'=>array('controller'=>'clientpages','action'=>'album',$value['Media']['defunt_id'],$value['Media']['album_id']),
					'title'=>__('voir les photos de cet album')));
				echo '<p>'.$value['Album']['name'].'</p>';
				echo '</div>';
			}
			}
...
<!-- <div class="row">
	...
	<?php echo $this->Html->link(
                        ...
                        array('title'=>'bouton pour revenir à la page précédente','class'=>'button')
                        ); ?>
                    </div>
                       </div>
                   </div>
