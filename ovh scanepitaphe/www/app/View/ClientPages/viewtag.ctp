	...
	<div class="small-12 medium-8 large-8 columns">
		<?php
			foreach ($medias as $key => $value) {
				...
				}elseif ($value['Media']['type'] == 'typeimage'){
                ...
                array('confirm' => __('Etes-vous sûr ?'))).'</a>';
					...
					if (!empty($value['Media']['nomfichier'])) {
						echo $this->Html->media(
								...
								foreach ($value['Tag'] as $key => $v) {
						if(!empty($v['title'])){
							echo '<span class="black radius secondary label" style="margin:1%;">'.$this->Html->link($value['title'],array('controller'=>'clientpages','action'=>'viewtag',$personne['Defunt']['id'],$v['id'])).'</span>';
						}
					}
					if($value['Media']['comment']==1){
						$comments = $this->requestAction('comments/getcomments/'.$value['Media']['id']);
