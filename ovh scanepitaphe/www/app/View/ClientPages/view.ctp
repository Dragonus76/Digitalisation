...
<?php else : ?>
<div class="row footgrey headpage imghead">
	<div class="small-12 medium-6 large-6 columns">
		<?php
...
<div class="row">
	<div class="small-12 medium-4 large-4 columns">
		<ul>
			<?php 
				...
				echo $this->Html->link(__('Galerie photos'),
						array(
	...
	<div class="small-12 medium-8 large-8 columns">
		<?php
			foreach ($timeline as $key => $value) {
				...
				if(!empty($value['Album']['name'])){
					...
					echo '<h3><small>'.strftime('%d %B %Y',strtotime($value['Album']['date'])).'</small> '.__('Album ');
					...
					if (!empty($cover['Media']['nomfichier'])) {
						...
						echo '<div class="legende"><p>'.__('Album ').$value['Album']['name'].'</p></div>';
						echo '</div>';
				...
				if ($value['Media']['type'] == 'typevideo'){
					...
					if($value['Media']['comment']==1){
						...
						foreach ($comments as $key => $val) {
							echo '<div class="row">';
							echo '<div class="small-3 medium-3 large-3 columns imgcomment">';
							$commentdate=strftime('%d %B %Y',strtotime($val['Comment']['created']));
							$usercomment = $this->requestAction('users/getuserbyusername/'.$val['Comment']['autor_name']);

							if(!empty($usercomment['User']['avatar'])){
								echo $this->Html->image('medias/users/'.'user_'.$usercomment['User']['id'].'/'.$usercomment['User']['avatar'],array(
									'width'=>50,
									'height'=>50));
                ...
                array('confirm' => __('Etes-vous sûr ?'))).'</a>';
							}	
						}
							echo '</div></div>';						
						}
						...
						echo '<div class="small-11 medium-11 large-11 columns">';
							...
							echo $this->Form->input('content', array('label'=>'','placeholder'=>__('écrire un commentaire')));
							echo '</div><div class="small-1 medium-1 large-1 columns">';
							echo $this->Form->submit(__('Ok'),array('class'=>'buttonvioletok postfix')); 
				            echo $this->Form->end();
				            echo '</div></div>';
						}else{
							echo '<div class="row">';
						echo '<div class="small-12 medium-12 large-12 columns">';
							echo '<p>'.__('Vous devez être ').$this->Html->link(__('connecté'),array('controller'=>'users','action'=>'login')).__(' pour ajouter un commentaire').'</p>';
				            echo '</div></div>';
						}
       ...
       <embed src="/img/medias'. DS .'defunts'. DS .'defunt_'.$value['Media']['defunt_id'] . DS . 'pdf'.DS.$value['Media']['nomfichier'].'" type="application/pdf" />
					...
					echo '<a class="media" href="/img/medias'. DS .'defunts'. DS .'defunt_'.$value['Media']['defunt_id'] . DS . 'pdf'.DS.$value['Media']['nomfichier'].'">'.__('Télécharger le pdf').'</a> ';
					foreach ($value['Tag'] as $key => $v) {
						if(!empty($v['title'])){
							echo '<span class="black radius secondary label" style="margin:1%;">'.$this->Html->link($v['title'],array('controller'=>'clientpages','action'=>'viewtag',$personne['Defunt']['id'],$v['id'])).'</span>';
						}
