...
class CheckoutsController extends AppController {
    ...
    public function custom($id = null){
		...
		if(
			...
			$this->Session->setFlash('Vous n\'avez pas choisi de photo ou la photo n\'est pas au format .jpg',"default", array('class' => 'alert-box warning rounds'));
        return $this->redirect(array('controller'=>'checkouts','action'=>'cart'));
		}
    	$this->set(compact('vars','user'));

    }else{
        $this->Session->setFlash('Erreur dans le panier',"default", array('class' => 'alert-box warning rounds'));
        ...
        if (!$alreadyIn) {
            ...
            $this->Session->setFlash('Produit ajouté au panier',"default", array('class' => 'alert-box success radius'));
    ...
    public function delete_cart($id = null) {
        ...
        if ($this->Session->delete('Cart.' . $id)) {
            ...
            $this->Session->setFlash('Le produit a été enlevé du panier',"default", array('class' => 'alert-box success radius'));
    ...
    public function empty_cart() {
        ...
        $this->Session->setFlash('Votre panier a été vidé.',"default", array('class' => 'alert-box success radius'));   	
    ...
    public function checkout1(){
        ...
        $this->Session->setFlash('Vous n\'avez pas renseigné toutes vos coordonnées.',"default", array('class' => 'alert-box warning round'));       
    ...
    public function checkout2(){
    	...
    	if(!empty($this->request->data)){
    		if ($this->request->is('post')) {
				...
				$this->Session->setFlash('Erreur. Vérifier si vous avez accepté les CGV et CGU.',"default", array('class' => 'alert-box success radius'));
 				return $this->redirect(array('controller'=>'checkouts', 'action' => 'checkout2'));
		          }	    		
    ...
    public function review(){
       ...
       //              ));
        ...
        if(!empty($cart)){
            ...
            $this->Session->setFlash('Erreur de session. Vous n\'avez pas été débité.',"default", array('class' => 'alert-box warning rounds'));
        ...
        if($this->Paypal->doExpressCheckoutPayment($orderp, $token, $PayerID)['ACK']=='SuccessWithWarning' || $this->Paypal->doExpressCheckoutPayment($orderp, $token, $PayerID)['ACK']=='Success'){
            ...
            $CakeEmail->subject('Confirmation de votre achat Scanepitaphe (à conserver)');
            $CakeEmail->viewVars(
                $user['User'] +
                array(
                    // 'token' => $token,
    ...
    public function cancel(){
        ...
        $personnes=$this->Defunt->find('all',array(
            ...
            $this->Session->setFlash('Erreur dans le processus de paiement. Vous n\'avez pas été débité.',"default", array('class' => 'alert-box warning rounds'));
    ...
    public function merge(){
        ...
        if(empty($custom)){
            $this->Session->setFlash('Erreur dans la création de l\'image',"default", array('class' => 'alert-box warning rounds'));
            return $this->redirect(array('controller'=>'products','action'=>'scanshop'));
        }
