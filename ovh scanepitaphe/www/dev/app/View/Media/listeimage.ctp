...
<div class="row minha">
    <div class="row">
        <div class="small-12 medium-12 large-12 columns center">
            <h3>Liste des contenus image pour <?php echo $this->Html->link($personne['Defunt']['firstname'].' '.$personne['Defunt']['name'],array('controller'=>'defunts','action'=>'editprinc',$personne['Defunt']['id']),array(
    ...
    <div class="row">
      <div class="small-12 medium-12 large-12 columns">
        <p>Vous n'avez pas ajouté de contenu image</p>
        <?php
            echo '<p>Il vous reste '.$limit['Limit']['picture'].' image(s) gratuite(s) à ajouter.</p>';
            echo '<p><span class="custom-label bgviolet">'.$this->Html->link('Ajouter une image',array('controller'=>'media','action'=>'addimageind',$personne['Defunt']['id'])).'</span></p>';
        ?>
      </div>
    </div>
</div>
...
<div class="row minha">
            ...
            <tr>
             ...
             <?php
                ...
                foreach ($medias as $key => $value) {
                    ...
                    echo '<td>'.$this->Html->link('Modifier',array('controller'=>'media','action'=>'editimage',$value['Media']['id'])).'</td>';
                    echo '<td>'.$this->Html->link("Supprimer ce contenu",array('controller'=>'media','action'=>'deleteimage',$value['Media']['id']),null,'Voulez vous vraiment supprimer ce contenu ? Cette action est DEFINITIVE.');
                    echo ' - ';
    ...
    <?php unset($posts); ?>
        ...
        if($limit['Limit']['text'] == 0){
                echo '<p>Vous avez atteint votre nombre d\'images autorisées. Rendez-vous dans le Scanshop (bientôt disponible) pour augmenter ce nombre.</p>';
            }else{
            echo '<p>Il vous reste '.$limit['Limit']['picture'].' image(s) gratuite(s) à ajouter.</p>';
            echo '<p><span class="custom-label bgviolet">'.$this->Html->link('Ajouter une image',array('controller'=>'media','action'=>'addimageind',$personne['Defunt']['id'])).'</span></p>';
        }
